...
def evaluate_accuracy(net, env, pc, n_trials=500, lrate=0.0, seed=0):
    random.seed(seed)
    ...
    for _ in range(n_trials):
        ...
        predicted, actual, sq_err, moved = run_trial(net, env, action, pc, lrate=lrate)
    ...
    print(f'Exact-cell decode accuracy over {n_trials} trials (lrate={lrate}): {acc:.2f}% ({correct}/{n_trials})')
